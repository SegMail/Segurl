...
function writeSoapCall() {
    var options = [];
    if ($('#url').val()) {
        //options.push('  url: "' + $('#url').val() + '"');
        options.push(' url: http://'+window.location.hostname+'/')
    }
    if ($('#method').val()) {
        options.push('  method: "' + $('#method').val() + '"');
    }
    if ($('#appendMethodToURL').prop('checked') == false) {
        options.push('  appendMethodToURL: false');
    }
    if ($('#SOAPAction').val()) {
        options.push('  SOAPAction: "' + $('#SOAPAction').val() + '"');
    }
    if ($('#soap12').prop('checked') == true) {
        options.push('  soap12: true');
    }
    if ($('#async').prop('checked') == false) {
        options.push('  async: false');
    }
    var HTTPHeaders = $('#HTTPHeaders').val();
    HTTPHeaders = HTTPHeaders.replace(/\n/g, '\n  ');
    if ($('#HTTPHeaders').val()) {
        options.push('  HTTPHeaders: ' + HTTPHeaders);
    }
    var envAttributes = $('#envAttributes').val();
    envAttributes = envAttributes.replace(/\n/g, '\n  ');
    if ($('#envAttributes').val()) {
        options.push('  envAttributes: ' + envAttributes);
    }
    var SOAPHeader = $('#SOAPHeader').val();
    ...
    if ($('#SOAPHeader').val()) {
        options.push('  SOAPHeader: ' + SOAPHeader);
    }
    ...
    if ($('#data').val()) {
        options.push('  data: ' + prettyData);
    }
    if ($('#namespaceQualifier').val()) {
        options.push('  namespaceQualifier: "' + $('#namespaceQualifier').val() + '"');
    }
    if ($('#namespaceURL').val()) {
        options.push('  namespaceURL: "' + $('#namespaceURL').val() + '"');
    }
    if ($('#noPrefix').prop('checked') == true) {
        options.push('  noPrefix: true');
    }
    if ($('#elementName').val()) {
        options.push('  elementName: "' + $('#elementName').val() + '"');
    }
    if ($('#enableLogging').prop('checked') == true) {
        options.push('  enableLogging: true');
    }
    if ($('#enableWSS').prop('checked')) {
        options.push('  wss: {\n    username: "' + $('#wssUsername').val() + '"');
